    ...
    merge: writeProcedure
        ...
        .handler(async ({ input, context }) => {
            ...
            const incidentsById = new Map(
                incidents.map((item) => [item.id, item]),
            );
            ...
            const status = getMergedIncidentStatus(
                target,
                mergedIncidents,
                endedAt,
            );
            const now = new Date();
            const targetMonitorIds = target.monitors.map(
                (item) => item.monitorId,
            );
            ...
            const allStatusPageIds = mergedIncidents.flatMap((item) =>
                item.statusPages.map(
                    (statusPageLink) => statusPageLink.statusPageId,
                ),
            );
